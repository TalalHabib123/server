...
function Condition_Cov() {
    ...
    function SaveCoverage(coverage, name, code, Spaces) {
        let flag = false;
        Condition_Coverage.forEach((stat) => {
            if (stat.name === name) {
                flag = true;
                return;
            }
    ...
    function ExecuteTestCases(Arguments, Function_Name, FunctionCalls, If_Spaces) {
        const code = readFileSync("../server/Utils/Temp/instrumented_code_c.js", "utf8");
    ...
    function Select_Best_Test_Case(Function_Order, Function_Name, Spaces) {
        ...
        Spaces.forEach((space, index) => {
            const index2 = Condition_Coverage.findIndex((stat) => stat.name === Function_Order[index]);
            const length_of_space = Object.keys(Condition_Coverage[index2].coverage).length;
            for (let i = 0; i < length_of_space; i++) {
                Condition_Coverage[index2].Actual_Coverage[i] = Coverage_Mapping[index_b];
                index_b++;
            }
        });
        let totalBranches = 0;
    ...
    DeadFunctions.forEach((func) => {
        ...
        if (Falg === true) {
            ...
            function saveInstrumentedCode(instrumentedCode) {
                const tempFilePath = `../server/Utils/Temp/instrumented_code_c.js`;
