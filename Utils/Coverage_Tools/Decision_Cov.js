...
function Decision_Cov() {
    ...
    function SaveCoverage(coverage, name, code, Spaces) {
        let flag = false;
        Decision_Coverage.forEach((stat) => {
            if (stat.name === name) {
                flag = true;
                return;
            }
        ...
        Spaces.forEach((space, index) => {
            if (space === 2) {
                removeAndFillObject(stat_coverage.b, index + 1);
            }
        });
        Decision_Coverage.push({
            name: name,
    ...
    function ExecuteTestCases(Arguments, Function_Name, FunctionCalls, If_Spaces) {
        const code = readFileSync("../server/Utils/Temp/instrumented_code_d.js", "utf8");
        ...
        If_Spaces.forEach((space, index) => {
            space.forEach((space2, index2) => {
                if (space2 === 2) {
                    removeAndFillObject(branch_covrage.b, index2 + 1);
                }
                index_b++;
            });
        });
    ...
    function Select_Best_Test_Case(Function_Order, Function_Name, Spaces) {
        ...
        for (const key in Test_Cases[0].coverage) {
            Coverage_Mapping[key] = [0, 0];
        }
        Temp_Test_Cases.forEach((test) => {
            for (const key in test.coverage) {
                if (test.coverage[key][0] === 1) {
                    Coverage_Mapping[key][0] = 1;
                }
                if (test.coverage[key][1] === 1) {
                    Coverage_Mapping[key][1] = 1;
                }
            }
        });
        let totalBranches = 0;
        ...
        for (const key in Coverage_Mapping) {
            if (Coverage_Mapping.hasOwnProperty(key)) {
                totalBranches += Coverage_Mapping[key].length;
                for (const path of Coverage_Mapping[key]) {
                    if (path > 0) {
        ...
        let index_b = 0;
        Spaces.forEach((space, index) => {
            const index2 = Decision_Coverage.findIndex((stat) => stat.name === Function_Order[index]);
            const length_of_space = Object.keys(Decision_Coverage[index2].coverage).length;
            for (let i = 0; i < length_of_space; i++) {
                Decision_Coverage[index2].Actual_Coverage[i] = Coverage_Mapping[index_b];
                index_b++;
            }
        });
        Best_Test_Case.push({
            ...
            totalBranchesCovered: totalBranchesCovered,
            percentage: ((totalBranchesCovered / totalBranches) * 100),
            Test_Cases: Temp_Test_Cases,
        });
        //console.log((totalBranchesCovered / totalBranches) * 100);
    }
    ...
    DeadFunctions.forEach((func) => {
        ...
        if (Falg === true) {
            function saveInstrumentedCode(instrumentedCode) {
                const tempFilePath = `../server/Utils/Temp/instrumented_code_d.js`;
                try {
